...
date modified: 2025-06-24T23:45:10+04:00
tags:
  - concept/scale
---
# Types of scaling
...
But it can be upgraded so each tier does not communicate directly, but with an internal [[AWS Elastic Load Balancer (ELB)]], which gives abstraction and horizontal scaling. 
...
- Stateless and ephemeral - each time env is new and clean
...
- Don't reinvent the wheel, if you need other than compute, consider managed services, like [[AWS S3 Simple Storage Service]] for storage.
...
[[EC2 Auto Scaling Groups (ASG)]] is configured to scale based on the **queue length**. Instances take messages in front of the queue. Once message is retrieved, instances take data from [[AWS S3 Simple Storage Service]] based on the in-message info.
...
>
>Key component of [[IT Software Architectures]]

Related: [[AWS EventBridge]], [[Apache Kafka]]
