...
**Outputs** – how to and were does outputs, stdio, etc.
...
`cfn-init` – the script name, has simple configuration management system
...
CF Template has EC2 instance configuration as `AWS::EC2:Instance` with `UserData` which launches `cfn-init` and provides it with stack+region etc.
