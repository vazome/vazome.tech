...
So the **default** behavior is that Producer is only writing to a single leader broker. Consumer is only reading from single leader broker even though replicas are in place – meaning replicas here are only for [[Business Resilience and Reliability#High availability (HA)]].
> [!NOTE]
